...
if (process.env.DATABASE_URL) {
  ...
  } catch(e) { console.log('[DB] indisponível:', e.message); }
}
...
app.get('/api/cardapio', async (req, res) => {
  ...
  try {
    ...
    if (!data) return res.status(429).json({ erro: 'rate_limit' });
...
app.get('/api/cardapio/detalhes', async (req, res) => {
  ...
  for (let i = 0; i < ids.length; i += 4) {
    ...
    const results = await Promise.all(lote.map(async id => {
      try { const r = await httpGet('https://integracao.cardapioweb.com/api/partner/v1/orders/'+id, { 'X-API-KEY': CW_TOKEN, Accept: 'application/json' }, 10000); return tryJSON(r.body); }
      catch(e) { return null; }
    }));
